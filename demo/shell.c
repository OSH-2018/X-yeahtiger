...
static __attribute__ ((noinline)) __attribute__ ((__used__)) void do_fix(void* pmain) {
  ...
  ext_symbols[1] = (ptr_t)0x0000000000201014; // nm a.out | grep global_data
  ext_symbols[2] = (ptr_t)0x0000000000201018; // objdump -R fix.so | grep print_global
  ext_symbols[3] = (ptr_t)0x000000000000077a; // nm a.out | grep print_global
  ...
  fix_units[0] = (ptr_t)0x000000000000079b;     // nm a.out | grep is_prime
  fix_units[1] = (ptr_t)0x0000000000000340;     // nm fix.so | grep fix_is_prime
